...
export class EventService {
  ...
  public async createEvent(request: CreateEventDTO): Promise<any> {
    const ref = this.firebaseService.getRef();
    const mediaUrl = await this.handleFiles(request.media, 'media', ref.id);
    const mediaThumbnailUrl = await this.handleFiles(
      request.mediaThumbnail,
      'thumbnail',
      ref.id,
    );
    const eventData = new CreateEventFirebaseRequestDTO(
      ...
      request.description,
      Timestamp.fromDate(new Date(request.expirationDate)),
      request.isDeleted,
      request.isExclusive,
      mediaThumbnailUrl,
      mediaUrl,
      Timestamp.fromDate(new Date(request.startDate)),
      request.taggedUsers,
      request.title,
      request.type,
      request.viewCount,
  ...
  public async updateDuration(request: UpdateEventDurationDTO) {
    ...
    response.expirationDate =
      this.firebaseService.convertDateToTimestamp(expirationDate);
    return await firebaseResponse.ref.update(response);
  }
  ...
  private async handleFiles(
    ...
    return await this.firebaseService.uploadFileFromBase64(
      fieldName,
      fileBase64,
      id,
    );
